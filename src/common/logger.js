...
function _sanitizeObject(obj) {
  try {
    return JSON.parse(JSON.stringify(obj, (name, value) => {
      ...
      const removeFields = ['code', 'accessToken'];
 ...
 */
logger.decorateWithLogging = function (service, serviceName) {
  if (config.LOG_LEVEL !== 'debug') {
    return;
  }
  _.each(service, (method, name) => {
    const params = method.params || getParams(method);
    const signature = `${serviceName}#${name}`;
 ...
 * @param {String} serviceName the service name
 */
logger.buildService = function (service, serviceName) {
